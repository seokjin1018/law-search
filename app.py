...
import sqlite3 # <<<<<<<<<<< [추가] SQLite 라이브러리
...
# ===== [삭제] 유저 데이터 JSON 관련 함수들 =====
# USERS_FILE, load_users(), save_users() 함수는 모두 삭제합니다.
...
def match_keywords(text, keywords, mode):
    ...
    if not processed_keywords: return True
    if mode == "SINGLE": return processed_keywords[0] in text_no_space
    elif mode == "AND": return all(k in text_no_space for k in processed_keywords)
    elif mode == "OR": return any(k in text_no_space for k in processed_keywords)
    elif mode == "AND_OR":
        if len(processed_keywords) < 2: return processed_keywords[0] in text_no_space
...
def process_search(items, data, date_key):
    ...
    for item in items:
        ...
        if not match_keywords(text_blob, keywords, mode): continue
        if exclude_keywords(text_blob, exclude): continue
        results.append(item)
    if sort_by == "latest": results.sort(key=lambda x: normalize_date_for_sort(x.get(date_key, "")), reverse=True)
    elif sort_by == "oldest": results.sort(key=lambda x: normalize_date_for_sort(x.get(date_key, "")))
    total = len(results)
...
def get_criminal_articles():
    ...
    if not law or law not in criminal_laws_dict: return jsonify([])
...
def search_criminal_cases():
    ...
    if selected_law: filtered_rows = [row for row in filtered_rows if selected_law in row.get("참조조문", "")]
    if selected_article: filtered_rows = [row for row in filtered_rows if selected_article in row.get("참조조문", "")]
